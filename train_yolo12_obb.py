...
"""
YOLO12x-OBB训练框架
基于ultralytics官方API，适配8类交通事故检测任务
支持旋转框检测(OBB)和类别不平衡优化
"""
...
class YOLO12OBBTrainer:
    ...
    def get_training_args(self) -> dict:
        ...
        args = {
            ...
            'name': training_config.get('experiment_name', 'yolo12x_obb_experiment'),
    ...
    def train(self):
        ...
        try:
            ...
            if 'out of memory' in error_msg or 'oom' in error_msg or 'cuda' in error_msg:
                ...
                logger.error("  3. 使用更小的模型 (yolo12s/l)")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='YOLO12x-OBB训练框架')
    parser.add_argument('--config', type=str, default='config/train_yolo12x_obb_7class.yaml',
                       help='训练配置文件路径')
    ...
    try:
        # 创建训练器
        trainer = YOLO12OBBTrainer(args.config)
